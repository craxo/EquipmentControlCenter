...
/// <summary>
/// Manages service state and publishes state change events
/// </summary>
public class ServiceStateManager
{
        ...
        IConfiguration configuration)
    {
        _bus = bus;
        _logger = logger;
        _serviceId = configuration["ServiceConfig:ServiceId"] ?? $"cnc-{Guid.NewGuid()}";
        _serviceName = configuration["ServiceConfig:ServiceName"] ?? "cnc-service";
